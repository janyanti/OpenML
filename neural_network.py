...
class NN:
    ...
    def train(self, iterations=100000, cost_interval=100):
        for i in range (iterations):
            ...
            self.grads = backwards_propagate(self.X, self.Y, self.feed, self.layers, self.a_derivative, self.error)
...
def define_parameters(model):
    ...
    for layer in range (layers):
        ...
        n_h = model[int(pos)] # depth of the layer
...
def backwards_propagate(X, Y, cache, layers, derivative, loss_function='cross_entropy'):
    Y_h, cache = cache
    ...
    for layer in range(layers, 0, -1):
        pos = layer
        if (layer == layers):
            dZ =  output_error(Y_h, Y, loss_function)
            E = dZ # donetes error
...
def update_params(params, gradients, layers, learning_rate=0.0005):
    # note: dP is used to represent the partial derivates of biases/weights
    alpha = learning_rate
